...
const displayValue = (value: unknown, fieldType?: string, options?: string[]): React.ReactNode => {
  ...
  if ((fieldType === 'table' || Array.isArray(value)) && Array.isArray(value)) {
    ...
    return (
      <div className="mt-1 overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
              <th className="w-8 px-2 py-1.5 text-center text-slate-400">#</th>
              {cols.map(c => <th key={c} className="px-2.5 py-1.5 font-bold text-slate-700 dark:text-slate-200">{c}</th>)}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {(value as Array<Record<string, string>>).map((row, idx) => (
              <tr key={idx}>
                <td className="px-2 py-1.5 text-center font-medium text-slate-400">{idx + 1}</td>
                {cols.map(c => <td key={c} className="px-2.5 py-1.5 text-slate-800 dark:text-slate-200">{row[c] || '—'}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
