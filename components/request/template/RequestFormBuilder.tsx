...
const fieldTypes: Array<{ value: RequestFieldType; label: string }> = [
  ...
  { value: 'select', label: 'Danh sách chọn' }, { value: 'user', label: 'Người dùng' }, { value: 'file', label: 'Tệp đính kèm' },
];
...
const RequestFormBuilder: React.FC<Props> = ({ fields, dispatch, issues }) => {
  ...
  const add = () => {
    ...
    if (!label) return;
    const field = { key: createFieldKey(label, fields.map(item => item.key)), label, fieldType: newType, required: false, options: newType === 'select' ? [''] : [], sortOrder: fields.length + 1 };
  ...
  const update = (index: number, patch: Partial<RequestTemplateDraft['fields'][number]>) => {
    ...
    dispatch({ type: 'UPSERT_FIELD', field: { ...existing, ...patch, options: fieldType === 'select' ? (patch.options ?? existing.options) : [] } });
  };
