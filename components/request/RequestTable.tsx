...
const ApproverProgress: React.FC<{ item: RequestListItem }> = ({ item }) => (
  <div className="flex -space-x-2" aria-label={`${item.activeApprovers.length} người đang duyệt`}>
    {item.activeApprovers.slice(0, 4).map(approver => <span key={approver.id} title={approver.name} className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-emerald-100 text-[10px] font-bold text-emerald-800 dark:border-slate-900">{approver.name.slice(0, 1).toUpperCase()}</span>)}
