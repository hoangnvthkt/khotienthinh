...
import {
  ...
  Calendar, CalendarOff, DollarSign, FileSignature, MapPin, Bot, TrendingUp
} from 'lucide-react';
...
const MODULE_CONFIG = [
  { key: 'WMS' as const, icon: Package, label: 'KHO - Vật tư', shortLabel: 'KHO', gradient: 'from-emerald-500 to-emerald-600', shadow: 'shadow-emerald-500/30', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-900/30', border: 'border-emerald-200 dark:border-emerald-700', route: '/dashboard' },
  { key: 'HRM' as const, icon: Briefcase, label: 'NS - Nhân sự', shortLabel: 'NS', gradient: 'from-teal-500 to-cyan-600', shadow: 'shadow-teal-500/30', color: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-900/30', border: 'border-teal-200 dark:border-teal-700', route: '/hrm/employees' },
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggle, collapsed, setCollapsed }) => {
  ...
  const moduleNavMap: Record<AppKey, any[]> = {
    ...
    HRM: [
      ...
      { to: '/hrm/salary-3p', icon: TrendingUp, label: 'Lương 3P' },
