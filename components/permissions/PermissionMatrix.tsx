...
import { isDirectPermissionGrantAllowed, isPermissionActionScopeAllowed } from '../../lib/permissions/permissionService';
                  ...
                  {module.actions.map(action => {
                    ...
                    return (
                      <label
                        ...
                        className={`flex min-h-[42px] items-center gap-2 rounded-lg border px-2 py-1.5 text-[10px] font-bold transition ${checked ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'} ${!scopeAllowed || !directGrantAllowed ? 'opacity-45' : ''}`}
                        title={!directGrantAllowed ? `${action.permissionCode} chỉ được cấp qua HR/HR Manage` : scopeAllowed ? action.permissionCode : `${action.permissionCode} không hỗ trợ scope hiện tại`}
                      >
                        <input
                          type="checkbox"
                          checked={checked}
