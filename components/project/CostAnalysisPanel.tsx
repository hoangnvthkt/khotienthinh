...
const CostAnalysisPanel: React.FC<Props> = ({ constructionSiteId, projectId, onSelectCostItem }) => {
  ...
  if (loading) return <div className="text-center py-8 text-sm text-zinc-400 font-bold">Đang tải cây danh mục chi phí dự án...</div>;
    ...
    <div className="space-y-4">
        ...
        <div className="p-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
          <div>
            <h4 className="text-xs font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-1.5">
              <BarChart3 size={14} className="text-teal-700 dark:text-teal-400" /> Danh mục chi phí dự án (Đồng bộ Quản lý Hợp đồng)
            </h4>
          ...
          <button onClick={load} className="p-1.5 rounded-lg text-zinc-400 hover:text-teal-700 dark:hover:text-teal-400 transition-colors" title="Làm mới dữ liệu">
            <RefreshCw size={13} />
          </button>
        </div>
        ...
        {treeNodes.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-800/60 text-[10px] font-bold uppercase text-zinc-500 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800">
      ...
      {editingNode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-zinc-900 p-5 shadow-2xl space-y-4 border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-150">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-[10px] font-bold uppercase text-zinc-400 mb-1">Khoản mục</label>
                <div className="font-bold text-xs text-zinc-700 dark:text-zinc-200">
                  <span className="font-bold text-teal-700 bg-teal-50 dark:bg-teal-950/40 dark:text-teal-400 px-1.5 py-0.5 rounded mr-1.5">{editingNode.symbol}</span>
                  {editingNode.name}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="0"
                  className="w-full rounded-xl border border-zinc-200 bg-white p-2.5 text-sm font-mono font-bold outline-none focus:border-teal-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-white bg-teal-700 hover:bg-teal-800 disabled:opacity-50"
              >
