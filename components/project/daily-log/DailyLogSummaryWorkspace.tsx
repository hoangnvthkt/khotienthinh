...
export const DailyLogSummaryWorkspace: React.FC<Props> = ({ bundle, mode, ensureSummaryLog, onSaved, onSubmit, onPublish, onReturnAll }) => {
  ...
  const canReview = bundle.rollout.enabled && bundle.summaryLog?.status === 'submitted' && bundle.permissions.canApprove;
  const canSubmit = bundle.rollout.enabled && bundle.permissions.canSummarize && sourceBlockers === 0 && unresolved === 0 && cards.length > 0;
  ...
  return <section className="space-y-4" aria-label="Workspace tổng hợp theo khu vực">
    ...
    {mode === 'summarize' ? <div className="sticky bottom-0 flex flex-col-reverse gap-2 border-t border-slate-200 bg-white/95 py-3 backdrop-blur sm:flex-row sm:justify-end dark:border-slate-700 dark:bg-slate-900/95"><button type="button" disabled={busy || !canSubmit} onClick={() => save(false)} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-teal-700 px-5 text-sm font-bold text-teal-800 disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Lưu tổng hợp</button><button type="button" disabled={busy || !canSubmit} onClick={() => save(true)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 text-sm font-bold text-white disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Gửi CHT</button></div> : canReview ? <div className="sticky bottom-0 flex flex-col-reverse gap-2 border-t border-slate-200 bg-white/95 py-3 sm:flex-row sm:justify-end dark:border-slate-700 dark:bg-slate-900/95">{canReview && <button type="button" disabled={busy} onClick={onReturnAll} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-red-300 px-4 text-sm font-bold text-red-700 disabled:opacity-50"><RotateCcw size={15} /> Trả lại toàn bộ</button>}{bundle.rollout.enabled && canPublishDailyLogSummary({ log: bundle.summaryLog, canApprove: bundle.permissions.canApprove, canPublishProgress: bundle.permissions.canPublishProgress }) && <button type="button" disabled={busy} onClick={publish} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 text-sm font-bold text-white disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />} {bundle.rollout.mode === 'pilot' ? 'Đối chiếu thử nghiệm' : 'Duyệt & công bố'}</button>}</div> : null}
  </section>;
};
