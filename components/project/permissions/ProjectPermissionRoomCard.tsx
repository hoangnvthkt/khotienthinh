...
import { getProjectPermissionRoomActionLabel, type ProjectPermissionRoomCode, type ProjectRoomActionCode } from '../../../lib/permissions/projectPermissionRooms';
...
const ProjectPermissionRoomCard: React.FC<Props> = ({ room, onOpen }) => {
  ...
  return <button type="button" onClick={() => onOpen(room.roomCode)} className="group relative flex min-h-56 w-full flex-col rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 active:translate-y-0 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-indigo-700">
    ...
    <div className="mt-auto pt-4"><div className="flex flex-wrap gap-1.5">{primaryCounts.length > 0 ? primaryCounts.map(([action, count]) => <span key={action} className="rounded-lg border border-indigo-100 bg-indigo-50 px-2 py-1 text-[10px] font-bold text-indigo-700 dark:border-indigo-900/70 dark:bg-indigo-950/30 dark:text-indigo-200">{getProjectPermissionRoomActionLabel(room.roomCode, action as ProjectRoomActionCode)} {count}</span>) : <span className="rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-500 dark:bg-slate-700 dark:text-slate-300">Chưa gán quyền nghiệp vụ</span>}{room.fallbackOnlyUserCount > 0 && <span className="rounded-lg border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-800">{room.fallbackOnlyUserCount} user chỉ có PBAC</span>}</div>{room.missingRequiredActions.length > 0 && <div className="mt-3 flex items-start gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-2.5 py-2 text-[10px] font-bold text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-200"><AlertTriangle size={13} className="mt-0.5 shrink-0" />Thiếu người {room.missingRequiredActions.map(action => getProjectPermissionRoomActionLabel(room.roomCode, action)).join(', ')}</div>}</div>
  </button>;
};
