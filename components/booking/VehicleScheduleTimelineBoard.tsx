...
export const VehicleScheduleTimelineBoard: React.FC<Props> = ({
  ...
  }, [vehicles, selectedLocationId, selectedVehicleType, searchQuery, filterOnlyActive, events]);
  ...
  const kpi = useMemo(() => {
    ...
    return {
      ...
      totalTrips: events.filter(e => e.type === 'BOOKING').length,
    };
  }, [vehicles, events, nowMs]);
  ...
  // Helper to compute event position & width
  ...
  return (
    <div className="space-y-4">
      {/* 1. Header Control Bar & View Mode */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-xs border border-slate-200 dark:border-slate-700 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        {/* Left: View Mode Toggle & Date Navigation */}
        <div className="flex flex-wrap items-center gap-3">
          {/* Time View Mode Switcher */}
                    ...
                    >
                      ...
                      <div className="flex-1 relative min-h-[56px] flex items-center">
                        ...
                        {vehicleEvents.map((event) => {
                          ...
                          const isConfirmed = event.status === 'CONFIRMED' || event.status === 'DISPATCHED';
      ...
      {/* 5. Interactive Floating Hover Card Tooltip */}
      {hoveredEvent && hoverPos && (
        <div
          style={{
            position: 'fixed',
            left: `${Math.min(window.innerWidth - 320, Math.max(20, hoverPos.x - 150))}px`,
            top: `${Math.max(10, hoverPos.y - 140)}px`,
          }}
          className="z-50 w-72 bg-slate-900/95 text-white p-3.5 rounded-2xl shadow-2xl border border-slate-700 backdrop-blur-md text-xs pointer-events-none animate-in fade-in zoom-in-95 duration-150"
        >
          {hoveredEvent.type === 'MAINTENANCE' ? (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400">Khóa xe</span>
              </div>
            ...
            <div className="space-y-2">
              ...
              {hoveredEvent.destination && (
                <div className="text-[11px] text-slate-200 flex items-center gap-1.5 truncate">
                  ...
                  <span className="truncate">{hoveredEvent.destination}</span>
                </div>
              )}
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {selectedEventForModal.type === 'MAINTENANCE' ? 'Lịch trình bảo dưỡng trong hệ thống' : 'Thông tin chi tiết chuyến đi'}
                  </p>
                </div>
              </div>
              <button
      ...
      {/* 7. Modal Create Maintenance Period */}
