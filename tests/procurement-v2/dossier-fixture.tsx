...
const page: ProcurementV2Page = { items: [dossier, request], nextCursor: null,
  snapshotToken: 'fixture', asOf: '2026-09-23T00:00:00Z', stale: false,
  counters: [{ key: 'dossiers', count: 2, grain: 'document' }] };
...
function Fixture() {
  ...
  return <MemoryRouter>{selected ? <><ProcurementV2DemandDetailView dossier={selected}
    ...
    <ProcurementV2SupplyDialog open={open} demandLabel={selected.sourceCode}
      canPurchase={selected.sourceAdapter === 'project_material_request'}
      onClose={() => setOpen(false)} onPurchase={() => setOpen(false)} />
    ...
    onOpen={card => setSelected(card.id === dossier.id ? dossier : request)}
