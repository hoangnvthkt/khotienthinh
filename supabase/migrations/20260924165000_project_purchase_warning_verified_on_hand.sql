-- Require explicit matching cache and ledger evidence for purchase-proposal on-hand stock.
-- Missing records remain unknown; older availableQty behavior remains unchanged.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'onHandQty', case
      when requested.cache_qty is not null
       and ledger.on_hand_qty is not null
       and requested.cache_qty = ledger.on_hand_qty
       and requested.cache_qty >= 0
       and ledger.on_hand_qty >= 0
       and issue.id is null
       and coalesce(receipt.complete, true)
        then coalesce(ledger.on_hand_qty, 0)
      else null end,
