-- Centralize the warehouse scope used by WMS approve/complete commands so the
-- database follows the same source/target rule as the client.
create or replace function app_private.wms_transaction_has_action(
...
set search_path = ''
as $$
  select app_private.wms_has_action(
    p_permission_code,
    case
      when p_permission_code = 'wms.transaction.approve'
       and p_transaction_type = 'IMPORT'::public.transaction_type
        then p_target_warehouse_id
      when p_permission_code = 'wms.transaction.approve'
       and p_transaction_type = 'TRANSFER'::public.transaction_type
       and exists (
         select 1
         from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) line(value)
         where nullif(line.value ->> 'fulfillmentBatchId', '') is not null
       )
        then p_target_warehouse_id
      when p_permission_code = 'wms.transaction.approve'
        then p_source_warehouse_id
      when p_permission_code = 'wms.transaction.complete'
       and p_transaction_type in (
         'IMPORT'::public.transaction_type,
         'TRANSFER'::public.transaction_type
       )
        then p_target_warehouse_id
      else p_source_warehouse_id
    end,
    null,
    p_requester_id,
    p_assigned_user_id,
...
revoke all on function app_private.wms_transaction_has_action(
...
begin
  ...
  v_can_adjust := app_private.wms_transaction_has_action(
    case
      when v_tx.status = 'PENDING'::public.transaction_status
        then 'wms.transaction.approve'
      else 'wms.transaction.complete'
    end,
    v_tx.type,
...
revoke all on function public.update_transaction_items_for_receipt(text, jsonb)
  from public, anon;
grant execute on function public.update_transaction_items_for_receipt(text, jsonb)
  to authenticated, service_role;
...
begin
  ...
  if p_actor_user_id is not null and p_actor_user_id is distinct from v_user.id then
    raise exception 'actor user id must match the authenticated user'
      using errcode = '42501';
  end if;
  v_actor_user_id := v_user.id;

  v_can_sync := app_private.wms_transaction_has_action(
    'wms.transaction.complete', v_tx.type,
    v_tx.source_warehouse_id, v_tx.target_warehouse_id, v_tx.items,
    v_tx.requester_id, v_tx.approver_id, v_user.id
  );
...
revoke all on function public.sync_fulfillment_receipt_for_transaction(text, uuid)
  from public, anon;
grant execute on function public.sync_fulfillment_receipt_for_transaction(text, uuid)
  to authenticated, service_role;
