-- Let canonical WMS transaction command grants reach the status RPC while
-- retaining every legacy allow path during the controlled-revocation window.
...
revoke all on function public.process_transaction_status(text, public.transaction_status, uuid) from public, anon;
grant execute on function public.process_transaction_status(text, public.transaction_status, uuid) to authenticated, service_role;
