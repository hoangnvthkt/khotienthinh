...
begin
  ...
  return app_private.has_governed_hrm_permission(v_request_user_id, p_permission_code, 'global', '*')
    or (v_subject_user_id = v_request_user_id
...
revoke all on function app_private.hrm_can_access_leave_request(uuid,text,uuid) from public,anon;
grant execute on function app_private.hrm_can_access_employee_subject(uuid,text,uuid) to authenticated,service_role;
grant execute on function app_private.hrm_can_access_leave_request(uuid,text,uuid) to authenticated,service_role;
