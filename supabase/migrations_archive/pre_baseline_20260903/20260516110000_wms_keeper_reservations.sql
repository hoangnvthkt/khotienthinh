-- WMS keeper permissions, material request fulfillment mode, and reservation support.
...
create index if not exists idx_requests_source_status_created_date
  on public.requests (source_warehouse_id, status, created_date desc)
  where source_warehouse_id is not null;

create index if not exists idx_requests_site_status_created_date
  on public.requests (site_warehouse_id, status, created_date desc)
  where site_warehouse_id is not null;

create index if not exists idx_requests_related_transaction_id
  on public.requests (related_transaction_id)
...
begin
  ...
  v_can_approve :=
    ...
    or (
      ...
      and (
        (v_tx.type = 'IMPORT'::public.transaction_type and v_user.assigned_warehouse_id is not distinct from v_tx.target_warehouse_id)
