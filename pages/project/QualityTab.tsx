    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-3.5 sm:p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3.5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-600 dark:bg-teal-950/50 dark:text-teal-400 border border-teal-100 dark:border-teal-900/40">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="rounded-md bg-teal-50 dark:bg-teal-950/40 px-2 py-0.5 text-[10px] font-black uppercase text-teal-700 dark:text-teal-300 border border-teal-200/60 dark:border-teal-900/40 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
            {qualityCapabilities.canEdit && (
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-teal-600 hover:bg-teal-700 px-4 py-2.5 text-xs font-black text-white shadow-md shadow-teal-600/20 transition active:scale-95 whitespace-nowrap min-h-[40px] sm:min-h-0"
              >
        ...
        {/* Dải chỉ số KPI */}
        <div className="mt-3.5 grid grid-cols-2 gap-2 sm:grid-cols-4 pt-3 border-t border-slate-100 dark:border-slate-800/80">
          <button
            onClick={() => { setSubTab('all'); setStatusFilter(''); setSelectedWbsFilter(''); }}
            className={`p-2.5 sm:p-3 rounded-xl border text-left transition ${!statusFilter && subTab === 'all' ? 'border-teal-500 bg-teal-50/40 dark:bg-teal-950/30 ring-1 ring-teal-500' : 'border-slate-100 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/60 bg-slate-50/50 dark:bg-slate-900/50'}`}
          >
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tổng hồ sơ</div>
            <div className="text-lg sm:text-xl font-black text-slate-800 dark:text-white mt-0.5">{checklists.length}</div>
          </button>
          <button
            onClick={() => { setSubTab('actions'); setActionQueueFilter('submitted'); }}
            className={`p-2.5 sm:p-3 rounded-xl border text-left transition ${actionQueueFilter === 'submitted' && subTab === 'actions' ? 'border-teal-500 bg-teal-50/40 dark:bg-teal-950/30 ring-1 ring-teal-500' : 'border-teal-100/70 hover:bg-teal-50/30 dark:border-teal-900/40 bg-teal-50/20 dark:bg-teal-950/20'}`}
          >
            <div className="text-[10px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider">Chờ duyệt</div>
            <div className="text-lg sm:text-xl font-black text-teal-700 dark:text-teal-300 mt-0.5">{globalCounts.submitted}</div>
          </button>
          <button
            onClick={() => { setSubTab('all'); setStatusFilter('approved'); }}
            className={`p-2.5 sm:p-3 rounded-xl border text-left transition ${statusFilter === 'approved' && subTab === 'all' ? 'border-emerald-500 bg-emerald-50/40 dark:bg-emerald-950/30 ring-1 ring-emerald-500' : 'border-emerald-100/70 hover:bg-emerald-50/30 dark:border-emerald-900/40 bg-emerald-50/20 dark:bg-emerald-950/20'}`}
          >
            <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Đã duyệt</div>
            <div className="text-lg sm:text-xl font-black text-emerald-700 dark:text-emerald-300 mt-0.5">{globalCounts.approved}</div>
          </button>
          <button
            onClick={() => { setSubTab('actions'); setActionQueueFilter('returned'); }}
            className={`p-2.5 sm:p-3 rounded-xl border text-left transition ${actionQueueFilter === 'returned' && subTab === 'actions' ? 'border-rose-500 bg-rose-50/40 dark:bg-rose-950/30 ring-1 ring-rose-500' : 'border-rose-100/70 hover:bg-rose-50/30 dark:border-rose-900/40 bg-rose-50/20 dark:bg-rose-950/20'}`}
          >
            <div className="text-[10px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider">Trả lại / Cần sửa</div>
            <div className="text-lg sm:text-xl font-black text-rose-700 dark:text-rose-300 mt-0.5">{globalCounts.returned}</div>
          </button>
        </div>
      </div>

      {/* SEGMENTED SUB-TABS */}
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-3 sm:gap-6 px-1 sm:px-2 text-xs font-black overflow-x-auto no-scrollbar">
        <button
          onClick={() => setSubTab('actions')}
          className={`pb-2.5 sm:pb-3 relative flex items-center gap-1.5 sm:gap-2 transition-colors shrink-0 whitespace-nowrap ${subTab === 'actions' ? 'text-teal-700 dark:text-teal-400 font-black' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}
        >
          <ShieldCheck size={16} />
        ...
        <button
          ...
          className={`pb-2.5 sm:pb-3 relative flex items-center gap-1.5 sm:gap-2 transition-colors shrink-0 whitespace-nowrap ${subTab === 'all' ? 'text-teal-700 dark:text-teal-400 font-black' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}
        >
          <FileText size={16} />
        ...
        <button
          ...
          className={`pb-2.5 sm:pb-3 relative flex items-center gap-1.5 sm:gap-2 transition-colors shrink-0 whitespace-nowrap ${subTab === 'wbs_tree' ? 'text-teal-700 dark:text-teal-400 font-black' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}
        >
          <Folder size={16} />
      ...
      {subTab === 'all' && (
        <div className="space-y-3">
          {/* Tool bar */}
          <div className="rounded-2xl border border-slate-200/90 bg-white p-3 sm:p-3.5 shadow-xs dark:border-slate-800 dark:bg-slate-900 space-y-2.5">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <select
                ...
                className="w-full sm:w-64 max-w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 outline-none truncate dark:border-slate-700 dark:bg-slate-950 dark:text-white"
              >
            ...
            <div className="flex items-center gap-1.5 pt-0.5 overflow-x-auto no-scrollbar">
              ...
              <button
                ...
                className={`rounded-xl px-2.5 py-1 text-xs font-bold whitespace-nowrap shrink-0 transition ${!statusFilter ? 'bg-slate-800 text-white dark:bg-slate-200 dark:text-slate-900' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 hover:bg-slate-200'}`}
              >
                Tất cả ({checklists.length})
              </button>
              <button
                onClick={() => { setStatusFilter('submitted'); setCurrentPage(1); }}
                className={`rounded-xl px-2.5 py-1 text-xs font-bold whitespace-nowrap shrink-0 transition ${statusFilter === 'submitted' ? 'bg-teal-600 text-white' : 'bg-teal-50 text-teal-700 dark:bg-teal-950/40 dark:text-teal-300 hover:bg-teal-100/50'}`}
              >
                Chờ duyệt ({globalCounts.submitted})
              </button>
              <button
                onClick={() => { setStatusFilter('approved'); setCurrentPage(1); }}
                className={`rounded-xl px-2.5 py-1 text-xs font-bold whitespace-nowrap shrink-0 transition ${statusFilter === 'approved' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 hover:bg-emerald-100/50'}`}
              >
                Đã duyệt ({globalCounts.approved})
              </button>
              <button
                onClick={() => { setStatusFilter('returned'); setCurrentPage(1); }}
                className={`rounded-xl px-2.5 py-1 text-xs font-bold whitespace-nowrap shrink-0 transition ${statusFilter === 'returned' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 hover:bg-rose-100/50'}`}
              >
                Trả lại ({globalCounts.returned})
              </button>
              <button
                onClick={() => { setStatusFilter('draft'); setCurrentPage(1); }}
                className={`rounded-xl px-2.5 py-1 text-xs font-bold whitespace-nowrap shrink-0 transition ${statusFilter === 'draft' ? 'bg-slate-700 text-white' : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 hover:bg-zinc-200'}`}
              >
            ...
            <div className="bg-card border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden">
              ...
              <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-800">
                {paginatedChecklists.map(item => {
                  ...
                  return (
                    <div key={item.id} className="p-3.5 space-y-2.5 bg-white dark:bg-slate-900 transition active:bg-slate-50/70">
                      {/* Row 1: Code + Status Badge */}
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-mono text-xs font-black text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md shrink-0">
                          {item.code}
                        </span>
                        <div className="shrink-0 badge-no-squeeze">
                          <ErpStatusBadge status={item.status || 'draft'} label={STATUS_CONFIG[item.status || 'draft']?.label} tone={getQualityStatusTone(item.status || 'draft')} />
                        </div>
                      </div>
                      ...
                      <div>
                        <button
                          onClick={() => openChecklist(item, true)}
                          className="font-black text-sm text-slate-800 dark:text-white hover:text-teal-600 transition text-left line-clamp-2 leading-snug"
                        >
                          {item.title}
                        </button>
                      </div>
                      ...
                      <div className="space-y-1 text-xs text-slate-500 dark:text-slate-400">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-[11px] font-bold text-slate-400 shrink-0">Hạng mục:</span>
                          {linkedTask ? (
                            <span className="font-bold text-slate-700 dark:text-slate-300 truncate" title={taskLabel(linkedTask)}>
                              {taskLabel(linkedTask)}
                            </span>
                          ) : (
                            <span className="text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-950/40 px-1.5 py-0.5 rounded shrink-0">
                              Chưa gắn WBS
                            </span>
                          )}
                        </div>

                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-400 font-semibold pt-0.5">
                          {item.workLocation && (
                            <span className="inline-flex items-center gap-1">
                              <MapPin size={11} className="text-slate-400 shrink-0" /> <span className="truncate max-w-[130px]">{item.workLocation}</span>
                            </span>
                          )}
                          <span className="inline-flex items-center gap-1">
                            <Calendar size={11} className="text-slate-400 shrink-0" /> {formatDate(item.workDate || item.createdAt)}
                          </span>
                          {item.workSupervisor && (
                            <span className="inline-flex items-center gap-1">
                              <User size={11} className="text-slate-400 shrink-0" /> <span className="truncate max-w-[110px]">{item.workSupervisor}</span>
                            </span>
                          )}
                        </div>
                      </div>
                        ...
                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
                          {photoCount > 0 && (
                            <span className="inline-flex items-center gap-1 rounded bg-teal-50 dark:bg-teal-950/40 text-teal-700 dark:text-teal-300 px-1.5 py-0.5 border border-teal-200/50">
                              <ImageIcon size={10} /> {photoCount} ảnh
                            </span>
                          )}
                          {fileCount > 0 && (
                            <span className="inline-flex items-center gap-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-1.5 py-0.5">
                              <Paperclip size={10} /> {fileCount} file
                            </span>
                          )}
                          {photoCount === 0 && fileCount === 0 && <span className="text-slate-300 text-[11px]">—</span>}
                        </div>

                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => openChecklist(item, true)}
                            className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 transition min-h-[34px]"
                          >
                            Chi tiết
                          </button>
                          {canEdit && (
                          ...
                          {canSubmit && (
                            <button
                              onClick={() => setSubmittingChecklist(item)}
                              className="rounded-lg bg-teal-600 hover:bg-teal-700 px-3 py-1.5 text-xs font-black text-white shadow-xs transition min-h-[34px]"
                            >
                              Gửi
                            </button>
                          )}
                          {canApprove && (
                            <>
                              <button
                                onClick={() => handleStatusChange(item, 'returned')}
                                className="rounded-lg border border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-300 px-2 py-1.5 text-xs font-black min-h-[34px]"
                              >
                                Trả lại
                              </button>
                              <button
                                onClick={() => handleStatusChange(item, 'approved')}
                                className="rounded-lg bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-xs font-black text-white shadow-xs transition min-h-[34px]"
                              >
                                Duyệt
                              </button>
                            </>
                          ...
                          {canDelete && (
                            <button
                              onClick={() => handleDelete(item)}
                              className="rounded-lg p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 transition min-h-[34px]"
                              title="Xóa hồ sơ"
                            >
                              <Trash2 size={14} />
                            </button>
              ...
              {/* Pagination footer */}
              <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
                          ...
                          <div>
                            ...
                            {item.workLocation && (
                              <div className="mt-1 flex items-center gap-1 text-[11px] text-slate-400 font-semibold">
                                <MapPin size={11} className="text-slate-400 shrink-0" /> <span className="truncate">{item.workLocation}</span>
                              </div>
                            )}
                          </div>

                          {/* Row 3: Attachments + Date + Actions */}
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 flex-wrap">
                            <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500">
                              <span>{formatDate(item.workDate || item.createdAt)}</span>
                              <span>•</span>
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                onClick={() => openChecklist(item, true)}
                                className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 transition min-h-[32px]"
                              >
                                Chi tiết
                              </button>
                              {canEdit && (
                              ...
                              {canSubmit && (
                                <button
                                  onClick={() => setSubmittingChecklist(item)}
                                  className="rounded-lg bg-teal-600 hover:bg-teal-700 px-2.5 py-1 text-xs font-black text-white shadow-xs transition min-h-[32px]"
                                >
                                  Gửi
                                </button>
                              )}
                              {canApprove && (
                                <button
                                  onClick={() => handleStatusChange(item, 'approved')}
                                  className="rounded-lg bg-emerald-600 hover:bg-emerald-700 px-2.5 py-1 text-xs font-black text-white shadow-xs transition min-h-[32px]"
                                >
                                  Duyệt
                                </button>
                              )}
                              {canDelete && (
                                <button
                                  onClick={() => handleDelete(item)}
                                  className="rounded-lg p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 transition min-h-[32px]"
                                  title="Xóa"
                                >
                                  <Trash2 size={14} />
                                </button>
