        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground font-medium">Thống kê chi tiết nguyên giá, khấu hao và bảo hành tài sản.</p>
                </div>
            ...
            {/* Filter Bar */}
            <div className="bg-card p-4 rounded-2xl shadow-sm border border-border flex flex-wrap gap-4 items-end">
                <div className="space-y-1.5 flex-1 min-w-[200px]">
                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center">
                        <Calendar size={12} className="mr-1" /> Khoảng thời gian mua
                    </label>
                    <div className="flex items-center gap-2">
                        <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
                            className="flex-1 p-2 bg-muted/50 border border-border rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-500" />
                ...
                <div className="space-y-1.5 w-full md:w-48">
                    ...
                    <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}
                        className="w-full p-2 bg-muted/50 border border-border rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-500">
                ...
                <div className="space-y-1.5 flex-1 min-w-[200px]">
                    ...
                    <input type="text" placeholder="Tên hoặc mã TS..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                        className="w-full p-2 bg-muted/50 border border-border rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-rose-500" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-500/10 text-indigo-450 rounded-xl flex items-center justify-center border border-indigo-500/20">
                        <Landmark size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-muted-foreground uppercase">{summary.count} tài sản</p>
                        <p className="text-lg font-black text-foreground dark:text-white">{summary.totalOriginal.toLocaleString('vi-VN')} ₫</p>
                        <p className="text-[9px] text-muted-foreground font-bold">Tổng nguyên giá</p>
                    </div>
                </div>
                <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                    <div className="w-12 h-12 bg-rose-500/10 text-rose-450 rounded-xl flex items-center justify-center border border-rose-500/20">
                        <TrendingUp size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-muted-foreground uppercase">Đã khấu hao</p>
                        <p className="text-lg font-black text-rose-600">{Math.round(summary.totalDepreciated).toLocaleString('vi-VN')} ₫</p>
                        <p className="text-[9px] text-muted-foreground font-bold">{summary.totalOriginal > 0 ? ((summary.totalDepreciated / summary.totalOriginal) * 100).toFixed(1) : 0}% nguyên giá</p>
                    </div>
                </div>
                <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted-foreground uppercase">Giá trị còn lại</p>
                        <p className="text-lg font-black text-emerald-600">{Math.round(summary.totalRemaining).toLocaleString('vi-VN')} ₫</p>
                        <p className="text-[9px] text-muted-foreground font-bold">{summary.totalOriginal > 0 ? ((summary.totalRemaining / summary.totalOriginal) * 100).toFixed(1) : 0}% nguyên giá</p>
                    </div>
                </div>
                <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${summary.warrantyExpiring > 0 ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-muted text-muted-foreground border-border'}`}>
                        <Shield size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-muted-foreground uppercase">BH sắp hết</p>
                        <p className={`text-lg font-black ${summary.warrantyExpiring > 0 ? 'text-amber-600' : 'text-muted-foreground'}`}>{summary.warrantyExpiring}</p>
                        <p className="text-[9px] text-muted-foreground font-bold">trong 30 ngày tới</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
                <div className="overflow-x-auto scrollbar-hide">
                    <table className="w-full text-left border-collapse min-w-[1400px]">
                        <thead>
                            <tr className="bg-muted/50 border-b border-border text-[10px] uppercase font-black text-muted-foreground tracking-wider">
                                <th className="p-4 sticky left-0 bg-muted z-10 border-r border-border">Tài sản / Mã</th>
                                <th className="p-4">Loại</th>
                                <th className="p-4 text-center">Trạng thái</th>
                                <th className="p-4 text-center">Ngày mua</th>
                                <th className="p-4 text-right bg-indigo-500/5">Nguyên giá</th>
                                <th className="p-4 text-center">KH (năm)</th>
                                <th className="p-4 text-right">KH/tháng</th>
                                <th className="p-4 text-right bg-rose-500/5 font-bold text-rose-450">KH lũy kế</th>
                                <th className="p-4 text-right bg-emerald-500/5 font-bold text-emerald-400">Còn lại</th>
                        ...
                        <tbody className="divide-y divide-border text-sm">
                            {reportData.map(row => (
                                <tr key={row.id} className="hover:bg-muted/30 transition-colors group">
                                    <td className="p-4 sticky left-0 bg-card group-hover:bg-muted/30 z-10 border-r border-border">
                                        <div className="font-bold text-foreground dark:text-white">{row.name}</div>
                                        <div className="text-[10px] font-mono text-muted-foreground">{row.code}</div>
                                    </td>
                                    <td className="p-4 text-xs font-bold text-muted-foreground">{row.category}</td>
                                    <td className="p-4 text-center">
                                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                                            row.status === AssetStatus.AVAILABLE ? 'bg-emerald-500/10 text-emerald-450' :
                                            row.status === AssetStatus.IN_USE ? 'bg-blue-500/10 text-blue-450' :
                                            row.status === AssetStatus.MAINTENANCE ? 'bg-amber-500/10 text-amber-500' :
                                            row.status === AssetStatus.DISPOSED ? 'bg-muted text-muted-foreground' :
                                            'bg-destructive/10 text-destructive'
                                        }`}>
                                            {ASSET_STATUS_LABELS[row.status]}
                                        </span>
                                    </td>
                                    <td className="p-4 text-center text-xs text-muted-foreground font-bold">{new Date(row.purchaseDate).toLocaleDateString('vi-VN')}</td>
                                    <td className="p-4 text-right font-bold text-foreground dark:text-slate-300 bg-indigo-500/5">
                                        {row.originalValue.toLocaleString('vi-VN')}
                                    </td>
                                    <td className="p-4 text-center text-muted-foreground font-bold">{row.depreciationYears}</td>
                                    <td className="p-4 text-right text-muted-foreground">{Math.round(row.monthlyDep).toLocaleString('vi-VN')}</td>
                                    <td className="p-4 text-right font-black text-rose-600 bg-rose-500/5">
                                        {Math.round(row.accumulatedDep).toLocaleString('vi-VN')}
                                        <div className="text-[9px] text-muted-foreground font-bold">{row.depPercent.toFixed(1)}%</div>
                                    </td>
                                    <td className="p-4 text-right font-black text-emerald-600 bg-emerald-500/5">
                                        {Math.round(row.remainingValue).toLocaleString('vi-VN')}
                                    </td>
                                    <td className="p-4 text-center">
                                        {row.warrantyMonths > 0 ? (
                                            <div>
                                                <span className={`text-[10px] font-black ${row.warrantyDaysLeft <= 0 ? 'text-muted-foreground' : row.warrantyDaysLeft <= 30 ? 'text-red-500' : 'text-emerald-500'}`}>
                                                    {row.warrantyDaysLeft <= 0 ? 'Hết' : `${row.warrantyDaysLeft}d`}
                                                </span>
                                                {row.warrantyExpiry && (
                                                    <div className="text-[9px] text-muted-foreground">{row.warrantyExpiry.toLocaleDateString('vi-VN')}</div>
                                                )}
                                            </div>
                                        ) : (
                                    ...
                                    <td className="p-4 text-xs text-muted-foreground font-medium">{row.assignedTo}</td>
                                </tr>
                            ))}
                            {reportData.length === 0 && (
                                <tr>
                        ...
                        {reportData.length > 0 && (
                            <tfoot>
                                <tr className="bg-slate-900 text-white font-black text-sm border-t-2 border-border">
                                    <td colSpan={4} className="p-4 sticky left-0 bg-slate-900 z-10">
