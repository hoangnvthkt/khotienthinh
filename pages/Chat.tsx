...
const MOCK_SERVERS = [
    { id: 'dm', name: 'Direct Messages', icon: '💬', color: 'from-indigo-500 to-purple-600' },
    { id: 'fc-erp', name: 'FastCons Corporate', icon: 'FC', color: 'from-amber-500 to-orange-600' },
    { id: 'rico', name: 'Công trường RICO', icon: 'RC', color: 'from-blue-500 to-indigo-600' },
    { id: 'tech', name: 'Đội Kỹ thuật', icon: 'TC', color: 'from-emerald-500 to-teal-600' },
    { id: 'finance', name: 'Kế toán & Tài chính', icon: 'KT', color: 'from-pink-500 to-rose-600' },
];

const MOCK_VOICE_USERS: Record<string, { name: string; avatar?: string }[]> = {
    'phong-hop': [
        { name: 'Nguyễn Văn Hoàng', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=60' },
        { name: 'Trần Thị Mai', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=60' }
    ],
    'ky-thuat': [
        { name: 'Phạm Đức Anh', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=60' }
    ],
    'truc-ban': []
};
...
const Chat: React.FC = () => {
    ...
    const [activeServer, setActiveServer] = useState<string>('dm'); // 'dm' or mock server id
    ...
    const filteredConversations = useMemo(() => {
        ...
        // Filter by server categories mapping (group chats go to mock servers, DMs go to 'dm')
        if (activeServer === 'dm') {
            list = conversations.filter(c => c.type === 'direct');
        } else {
            // Group chats show under custom workspaces
    ...
    const playSound = (type: 'connect' | 'disconnect' | 'click' | 'theme') => {
        try {
    ...
    const handleRenameGroup = async () => {
        ...
        try {
            ...
            alert(getErrorMessage(err, 'Không thể đổi tên nhóm.'));
        }
    };
        ...
        <div className={`h-[calc(100vh-65px)] flex overflow-hidden select-none transition-all duration-300 font-sans ${currentTheme.chatArea}`}>
            ...
            <div className={`w-[72px] flex flex-col items-center py-3 gap-2 shrink-0 select-none transition-all duration-300 border-r ${currentTheme.serverBar}`}>
                ...
                <div className="relative group flex items-center justify-center w-full">
                    <div className={`absolute left-0 w-1 rounded-r-md transition-all duration-350 ${
                        activeServer === 'dm' ? 'h-9 bg-current' : 'h-1.5 opacity-0 group-hover:opacity-100 group-hover:h-4 bg-current'
                    }`} />
                    <button onClick={() => { setActiveServer('dm'); playSound('click'); }}
                ...
                {/* WORKSPACES LIST */}
                {MOCK_SERVERS.filter(s => s.id !== 'dm').map(server => {
                    const isSelected = activeServer === server.id;
                    return (
                        <div key={server.id} className="relative group flex items-center justify-center w-full">
                            <div className={`absolute left-0 w-1 rounded-r-md transition-all duration-350 ${
                                isSelected ? 'h-9 bg-current' : 'h-1.5 opacity-0 group-hover:opacity-100 group-hover:h-4 bg-current'
                            }`} />
                            <button onClick={() => { setActiveServer(server.id); playSound('click'); }}
                            ...
                            <span className="absolute left-[80px] bg-slate-900 text-white font-bold text-xs py-1.5 px-3 rounded-lg shadow-xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50 whitespace-nowrap">{server.name}</span>
                        </div>
                    );
                })}
                ...
                <div className="relative group flex items-center justify-center w-full mt-auto">
                    <button onClick={() => { setShowNewGroup(true); playSound('click'); }}
                    ...
                    <span className="absolute left-[80px] bg-slate-900 text-white font-bold text-xs py-1.5 px-3 rounded-lg shadow-xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50 whitespace-nowrap">Tạo Đội nhóm / Nhóm chat</span>
                </div>
            </div>
            ...
            <div className={`w-full md:w-[240px] flex flex-col shrink-0 ${mobileShowChat ? 'hidden md:flex' : 'flex'} select-none relative transition-all duration-300 ${currentTheme.sidebar}`}>
                ...
                <div className={`h-[48px] border-b px-3.5 flex items-center justify-between font-bold text-xs shrink-0 transition-all duration-300 ${currentTheme.sidebarHeader}`}>
                    {activeServer === 'dm' ? (
                        ...
                        <div className="flex items-center justify-between w-full cursor-pointer p-1.5 rounded transition-all group hover:bg-current/5">
                            <span className="truncate font-extrabold">{MOCK_SERVERS.find(s => s.id === activeServer)?.name}</span>
                            <ChevronDown size={14} className="text-current opacity-60 group-hover:opacity-100" />
                        </div>
                    )}
                </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                <button onClick={() => setShowCategoryText(!showCategoryText)}
                                    className={`w-full flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-1 mb-1 transition-all ${currentTheme.categoryText}`}>
                                ...
                                {showCategoryText && (
                                    <div className="space-y-0.5 pl-1.5">
                                        {/* Main static Discord mapped group chats */}
                                        {filteredConversations.map(conv => {
                                            const isActive = conv.id === activeConversationId;
                                            const name = getConvName(conv);
                                            const unread = conv.unreadCount || 0;
                                        ...
                                        {/* Mock static defaults in case no groups */}
                                        <button className={`w-full flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs text-left transition-all ${currentTheme.sidebarItemInactive}`}>
                                            <Hash size={14} className="opacity-50 shrink-0" />
                                            <span className="truncate">thông-báo-chung</span>
                                        </button>
                                        <button className={`w-full flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs text-left transition-all ${currentTheme.sidebarItemInactive}`}>
                                            <Hash size={14} className="opacity-50 shrink-0" />
                                            <span className="truncate">kỹ-thuật-kế-hoạch</span>
                                        </button>
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                {showCategoryVoice && (
                                    <div className="space-y-1 pl-1.5">
                                        {[
                                            { id: 'phong-hop', name: '🔊 Phòng họp chính' },
                                            { id: 'ky-thuat', name: '🔊 Đội kỹ sư RICO' },
                                            { id: 'truc-ban', name: '🔊 Trực kỹ thuật 24/7' }
                                        ].map(chan => {
                                            const isConnected = voiceConnected === chan.id;
                                            const voiceUsers = MOCK_VOICE_USERS[chan.id] || [];
                                            return (
                                                <div key={chan.id} className="space-y-0.5">
                                                    <button onClick={() => {
                                                        ...
                                                        {isConnected && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />}
                                                    </button>
                                                    
                                                    {/* Connected users list */}
                                                    {(isConnected || voiceUsers.length > 0) && (
                                                        <div className="pl-5 space-y-1 py-1">
                                                            {/* Current user connects visually */}
                                                            {isConnected && (
                                                            ...
                                                            {voiceUsers.map((vu, vi) => (
                                                                <div key={vi} className="flex items-center gap-1.5 py-0.5">
                                                                    <div className="w-4 h-4 rounded-full bg-slate-650 flex items-center justify-center text-white text-[8px] font-black overflow-hidden shrink-0">
                                                                        {vu.avatar ? <img src={vu.avatar} className="w-full h-full object-cover" /> : vu.name.charAt(0)}
                                                                    </div>
                                                                    <span className="text-[10px] opacity-75 truncate">{vu.name}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    )}
                        ...
                        <div className="bg-black/10 px-3 py-2 flex items-center justify-between border-b border-black/20">
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="text-[9px] opacity-70 truncate mt-0.5">
                                    {voiceConnected === 'phong-hop' ? 'Phòng họp chính' : voiceConnected === 'ky-thuat' ? 'Đội kỹ sư RICO' : 'Trực kỹ thuật 24/7'}
                                </div>
                            </div>
                    ...
                    <div className="p-2 flex items-center justify-between gap-1">
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                            <div className="relative shrink-0">
                                ...
                                <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500 border border-current" />
                            </div>
                            <div className="min-w-0 flex-1">
                                <div className="text-xs font-extrabold truncate">{user.name}</div>
                                <div className="text-[9px] opacity-60 truncate font-semibold uppercase tracking-wide">{user.role}</div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-0.5 shrink-0">
                            ...
                            <button className="w-7 h-7 rounded-md flex items-center justify-center opacity-70 hover:opacity-100 hover:bg-current/5 transition-colors"
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <h3 className="text-xs font-black truncate flex items-center gap-1">
                                        {activeConv.type === 'group' && <Hash size={12} className="opacity-50" />}
                                                    ...
                                                    <div key={msg.id}
                                                        ...
                                                        {!isMe && (
                                                            <div className="w-8 shrink-0 self-start mt-0.5">
                                                                {showAvatar && (
                                                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-650 flex items-center justify-center text-white text-xs font-black overflow-hidden shadow">
                                                        ...
                                                        <div className={`max-w-[70%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                                            {showName && (
                                                                <span className="text-[10px] font-black opacity-60 mb-0.5 ml-1 flex items-center gap-1.5">
                                                                    {sender?.name}
                                                                    {(sender?.role as any) === 'admin' && <Shield size={10} className="text-amber-500" />}
                                                                </span>
                                                            )}
                                                                        ...
                                                                        {REACTION_EMOJIS.map(emoji => {
                                                                            ...
                                                                            return (
                                                                                <button key={emoji}
                                                                                    ...
                                                                                    className={`w-7.5 h-7.5 rounded-lg flex items-center justify-center text-sm hover:bg-slate-100 dark:hover:bg-slate-700 hover:scale-120 transition-all ${myReacted ? 'bg-indigo-500/10 ring-1 ring-indigo-500/50' : ''}`}>
                                                                                    {emoji}
                                                                                </button>
                                                                            );
                                                                        })}
                                ...
                                <div className={`p-3 relative z-10 shrink-0 transition-all duration-300 ${currentTheme.inputPanel}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button onClick={() => setShowEmojiPicker(!showEmojiPicker)}
                                            className={`emoji-trigger w-9.5 h-9.5 rounded-xl flex items-center justify-center transition-all ${
                                        ...
                                        <button 
                                            ...
                                            className="w-9.5 h-9.5 rounded-xl flex items-center justify-center text-current opacity-70 hover:opacity-100 hover:bg-current/5 transition-colors"
                                        ...
                                        <div className={`flex-1 flex items-center rounded-xl px-4 py-2 border transition-all ${currentTheme.inputFieldBg}`}>
                                            <input ref={inputRef} value={msgInput}
                                                ...
                                                placeholder={activeConv.type === 'group' ? `Gửi tin nhắn vào #${getConvName(activeConv).toLowerCase().replace(/\s+/g, '-')}` : `Nhắn tin cho ${getConvName(activeConv)}`}
                                        ...
                                        <button onClick={handleSend} disabled={!msgInput.trim()}
                                            className="w-9.5 h-9.5 rounded-xl bg-indigo-550 flex items-center justify-center text-white shadow shadow-indigo-550/10 disabled:opacity-40 disabled:shadow-none hover:bg-indigo-650 transition-all active:scale-95 shrink-0">
                                            <Send size={15} />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className={`w-[280px] border-l flex flex-col shrink-0 overflow-y-auto animate-in slide-in-from-right duration-200 z-10 select-none transition-all duration-300 ${currentTheme.rightPanel}`}>
                                    ...
                                    <div className={`p-4 border-b flex items-center justify-between transition-all duration-300 ${currentTheme.rightPanelHeader}`}>
                                        <span className="text-xs font-black uppercase tracking-wider">
                                            {activeConv.type === 'group' ? 'Hồ sơ nhóm' : 'Hồ sơ thành viên'}
                                        </span>
                                    ...
                                    <div className={`p-5 flex flex-col items-center text-center transition-all duration-300 ${currentTheme.rightPanelProfile}`}>
                                        ...
                                        {activeConv.type === 'group' && editingGroupName ? (
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                <span className={`text-sm font-extrabold ${currentTheme.rightPanelProfileName}`}>{getConvName(activeConv)}</span>
                                                {activeConv.type === 'group' && (
                                        ...
                                        <span className="text-[10px] opacity-60 mt-1 font-semibold uppercase tracking-wide">
                                            {activeConv.type === 'group' ? `${activeConv.members?.length || 0} thành viên` : 'NHÂN VIÊN ERP'}
                                        </span>
                                    </div>
                                                    ...
                                                    <div className="mb-3 p-2 bg-black/10 rounded-xl space-y-1 max-h-[140px] overflow-y-auto">
                                                        {addableUsers.map(u => (
                                                            <button key={u.id} onClick={() => { void handleAddMember(u.id); }}
                                                                className="w-full flex items-center gap-2 p-1.5 rounded-lg hover:bg-current/5 transition-all text-left">
                                                                <div className="w-5.5 h-5.5 rounded-full bg-emerald-500 flex items-center justify-center text-white text-[8px] font-black shrink-0">
                                                                    {u.name?.charAt(0)?.toUpperCase()}
                                                                </div>
                                                                <span className="text-[10px] font-bold text-current flex-1 truncate">{u.name}</span>
                                                ...
                                                {(activeConv.members || []).map(member => {
                                                    ...
                                                    return (
                                                        <div key={member.id} className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-current/5 group">
                                                            <div className="relative shrink-0">
                                                                <div className="w-7 h-7 rounded-full bg-slate-600 flex items-center justify-center text-white text-[10px] font-bold overflow-hidden">
                                        ...
                                        {confirmDelete ? (
                                            <div className="p-2.5 bg-red-500/5 rounded-lg border border-red-500/30">
                                                <p className="text-[10px] font-bold text-red-500 mb-2 text-center">
                                                    {activeConv.type === 'group' ? 'Xóa nhóm khỏi danh sách hoạt động?' : 'Ẩn cuộc trò chuyện khỏi danh sách của bạn?'}
                                                </p>
                                                <div className="flex gap-1.5">
                                                    <button onClick={() => { void handleDeleteConversation(); }}
                                                        className="flex-1 py-1 rounded bg-red-500 text-white text-[10px] font-bold hover:bg-red-600">Xóa cuộc chat</button>
                                            ...
                                            <button onClick={() => setConfirmDelete(true)}
                                                ...
                                                <Trash2 size={14} /> <span className="text-[11px] font-bold">Xóa cuộc trò chuyện</span>
                                            </button>
                                        )}
                                    </div>
                                </div>
            ...
            {showCallOverlay && (
                <div className="absolute inset-0 bg-slate-950/95 flex flex-col items-center justify-center z-50 animate-in fade-in duration-200">
                    <div className="w-24 h-24 rounded-full bg-indigo-500 flex items-center justify-center text-white text-3xl font-black shadow-2xl relative mb-6">
                        ...
                        {/* Interactive animation rings */}
                        <div className="absolute inset-0 rounded-full bg-indigo-500/40 animate-ping duration-1000" />
                    </div>
                    
                    <h2 className="text-xl font-extrabold text-white mb-1">{activeConv ? getConvName(activeConv) : 'Cuộc gọi thoại'}</h2>
                    <p className="text-xs text-slate-400 mb-8 uppercase tracking-wider flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        {showCallOverlay === 'audio' ? 'Đang gọi thoại...' : 'Cuộc gọi Video bắt đầu'} • {formatCallTime(callDuration)}
                    </p>

                    {/* Simulation video viewport box if video call */}
                    {showCallOverlay === 'video' && (
                        <div className="w-[320px] h-[180px] bg-slate-900 border border-[#3f4147] rounded-2xl flex items-center justify-center relative mb-8 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                            <span className="text-xs text-slate-500">Camera của đối tác</span>
                            {/* Inner user camera preview thumbnail */}
                            <div className="absolute bottom-2 right-2 w-20 h-12 bg-slate-800 border border-slate-700 rounded-lg flex items-center justify-center text-[8px] text-slate-400">
                                Cam của bạn
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button onClick={() => { setIsDeafened(!isDeafened); playSound('click'); }}
                            className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
                                isDeafened ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
                            }`}>
