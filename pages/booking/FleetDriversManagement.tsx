...
import { CalendarOff, Eye, Plus, RefreshCw, ShieldCheck, UserRoundCog, X } from 'lucide-react';
import type {
...
import {
  ...
  getDriverAuthorizationEvidenceValidationError,
...
const FleetDriversManagement: React.FC<Props> = ({ fetchCandidates }) => {
  ...
  const openLicensePreview = async (driver: VehicleDriverAuthorizationAdminView) => {
    const title = `Bằng lái ${driver.employee_name || 'tài xế'}`;
    setEvidencePreview({ title, items: [] });
    try {
      const items = await resolvePrivateEvidencePreviewItems([
        { label: 'Mặt trước bằng lái', path: driver.license_front_photo_path },
        { label: 'Mặt sau bằng lái', path: driver.license_back_photo_path },
      ]);
      setEvidencePreview({
        title,
        items,
        error: items.length === 0 ? 'Không thể tải ảnh bằng lái. Vui lòng kiểm tra quyền truy cập hoặc thử lại.' : undefined,
      });
    } catch {
      setEvidencePreview({ title, items: [], error: 'Không thể tải ảnh bằng lái. Vui lòng thử lại.' });
    }
  };
  ...
  const saveDriver = async (event: React.FormEvent) => {
    ...
    const evidenceError = getDriverAuthorizationEvidenceValidationError({
      licenseFrontPhotoPath: frontFile ? 'pending-upload' : editing?.license_front_photo_path,
      licenseBackPhotoPath: backFile ? 'pending-upload' : editing?.license_back_photo_path,
    });
    if (evidenceError) {
      toast.error(evidenceError === 'LICENSE_FRONT_PHOTO_REQUIRED'
        ? 'Vui lòng đính kèm ảnh mặt trước bằng lái.'
        : 'Vui lòng đính kèm ảnh mặt sau bằng lái.');
      return;
    }
    try {
      setSaving(true);
