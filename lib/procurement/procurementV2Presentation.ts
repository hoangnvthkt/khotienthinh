...
export const resolveDossierSourceRoute = (source: { adapter: ProcurementV2Source; id: string }): string => {
  if (!source.id.trim()) throw new Error('PROCUREMENT_SOURCE_REF_INVALID');
  const id = encodeURIComponent(source.id);
  return source.adapter === 'material_plan' ? `/project-v2/plans/${id}` : `/rq/${id}`;
};
