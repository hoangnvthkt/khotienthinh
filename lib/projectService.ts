...
const mapPage = <T>(
    ...
    return {
        ...
        hasMore,
        nextCursor: hasMore && rawRows[limit - 1] ? getCursor(rawRows[limit - 1]) : null,
    };
};

const loadAllPages = async <T>(loadPage: (cursor: string | null) => Promise<ListPage<T>>): Promise<T[]> => {
    return fetchAllPages<T, string>({
    ...
    for (const valueChunk of chunkValues(Array.from(new Set(input.values.filter(Boolean))), 100)) {
        ...
        while (true) {
            let query: any = supabase
                ...
                .limit(PROJECT_PROCUREMENT_PAGE_SIZE);
            if (lastId) query = query.gt('id', lastId);
            const { data, error } = await query;
            if (error) throw error;
            const page = data || [];
            rows.push(...page);
            if (rows.length > 20_000) throw new Error('Project procurement read exceeded safety cap of 20000 rows');
            if (page.length < PROJECT_PROCUREMENT_PAGE_SIZE) break;
            const nextId = String(page[page.length - 1]?.id || '');
            if (!nextId || nextId === lastId) throw new Error('Project procurement read received a repeated cursor');
            lastId = nextId;
        }
    }
    return rows;
...
const poDeliveryBatchToDb = (batch: PurchaseOrderDeliveryBatch): any => {
    const row = toDb({
        ...
        approvalAssigneeUserId: batch.approvalAssigneeUserId || null,
...
const poDeliveryBatchFromRows = (batch: any, lineRows: any[]): PurchaseOrderDeliveryBatch => ({
    ...
    approvalAssigneeUserId: batch.approval_assignee_user_id || null,
...
const listPurchaseOrdersPage = async (input: {
    ...
    const cursor = readCreatedAtCursor(input.cursor);
    let query = supabase
    ...
    const { data, error } = await query;
    if (error) throw error;
    const page = mapPage(data || [], limit, poFromDb, writeCreatedAtCursor);
...
const listPurchaseOrderRequestLineLinksPage = async (input: {
    ...
    let query = supabase
        ...
        .select(PURCHASE_ORDER_REQUEST_LINE_SELECT)
        .or(buildProjectScopeFilter(input.projectIdOrSiteId, input.constructionSiteId))
        .order('created_at', { ascending: false })
        .order('id', { ascending: false })
        .limit(limit + 1);
    ...
    const { data, error } = await query;
    if (error) throw error;
    return mapPage(data || [], limit, fromDb, writeCreatedAtCursor);
};
...
export const poService = {
    async saveAggregate(input: SavePurchaseOrderAggregateInput): Promise<SavePurchaseOrderAggregateResult> {
        const purchaseOrder = poToDb(input.purchaseOrder);
        const deliveryBatches = input.deliveryBatches.map((batch, index) => {
            const normalizedBatch: PurchaseOrderDeliveryBatch = {
                ...batch,
                purchaseOrderId: input.purchaseOrder.id,
                projectId: input.purchaseOrder.projectId || null,
                constructionSiteId: input.purchaseOrder.constructionSiteId || null,
                deliveryNo: index + 1,
                status: batch.status || 'planned',
                fulfillmentMode: (batch.fulfillmentMode
                    || input.purchaseOrder.fulfillmentMode
                    || 'RECEIVE_TO_STOCK') as PurchaseOrderDeliveryBatch['fulfillmentMode'],
                fulfillmentBatchIds: batch.fulfillmentBatchIds || [],
                lines: (batch.lines || []).filter(line => Number(line.plannedQty || 0) > 0),
            };
            return {
                ...poDeliveryBatchToDb(normalizedBatch),
                lines: normalizedBatch.lines.map(line => poDeliveryLineToDb({
                    ...line,
                    deliveryBatchId: normalizedBatch.id,
                    purchaseOrderId: input.purchaseOrder.id,
                })),
            };
        }).filter(batch => batch.lines.length > 0);
        const { data, error } = await supabase.rpc('save_purchase_order_aggregate_v1', {
            p_purchase_order: purchaseOrder,
            p_request_line_links: input.requestLineLinks.map(poRequestLineLinkToDb),
            p_delivery_batches: deliveryBatches,
            p_expected_row_version: input.expected.rowVersion,
            p_expected_request_line_links: input.expected.requestLineLinks,
            p_expected_delivery_batches: input.expected.deliveryBatches,
    ...
    async listByIds(ids: string[]): Promise<PurchaseOrder[]> {
        ...
        const rows = await Promise.all(chunkValues(uniqueIds, 100).map(async idChunk => {
            const { data, error } = await supabase
                .from('purchase_orders')
                .select(PURCHASE_ORDER_SELECT)
                .in('id', idChunk)
        ...
        return rows.flat().map(fromDb) as PurchaseOrder[];
    },
...
export const poSupplementalApprovalService = {
    async listByPurchaseOrderIds(poIds: string[]): Promise<Record<string, PurchaseOrderSupplementalApproval[]>> {
        ...
        let rows: any[];
        try {
            rows = await loadProjectRowsByChunkedValue({
                table: 'purchase_order_supplemental_approvals',
                projection: PO_SUPPLEMENTAL_APPROVAL_SELECT,
                filterColumn: 'purchase_order_id',
                values: uniqueIds,
            });
            rows.sort((left, right) => String(right.created_at).localeCompare(String(left.created_at)) || String(right.id).localeCompare(String(left.id)));
        } catch (error: any) {
            if (error?.code === '42P01') return {};
            throw error;
        }

        return rows.reduce<Record<string, PurchaseOrderSupplementalApproval[]>>((acc, row) => {
...
export const poDeliveryScheduleService = {
    async listByPurchaseOrderIds(poIds: string[]): Promise<Record<string, PurchaseOrderDeliveryBatch[]>> {
        ...
        let batches: any[];
        try {
            batches = await loadProjectRowsByChunkedValue({
                table: 'purchase_order_delivery_batches',
                projection: PO_DELIVERY_BATCH_SELECT,
                filterColumn: 'purchase_order_id',
                values: uniqueIds,
            });
            batches.sort((left, right) => Number(left.delivery_no) - Number(right.delivery_no) || String(left.id).localeCompare(String(right.id)));
        } catch (error: any) {
            if (error?.code === '42P01') return {};
            throw error;
        }
        ...
        let lineRows: any[];
        try {
            lineRows = await loadProjectRowsByChunkedValue({
                table: 'purchase_order_delivery_lines',
                projection: PO_DELIVERY_LINE_SELECT,
                filterColumn: 'delivery_batch_id',
    ...
    async replaceForPurchaseOrder(po: PurchaseOrder, batches: PurchaseOrderDeliveryBatch[]): Promise<void> {
        ...
        if ((existingRows || []).length >= PROJECT_PROCUREMENT_PAGE_SIZE) {
            throw new Error(`PO delivery batch validation exceeded safety cap of ${PROJECT_PROCUREMENT_PAGE_SIZE} rows`);
        }
