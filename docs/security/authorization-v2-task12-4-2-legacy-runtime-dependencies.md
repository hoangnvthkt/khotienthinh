...
Sau E17 còn 10 function gọi trực tiếp literal `is_module_admin('WMS')`: hai helper delete cũ không còn lặp fallback, thay bằng một `material_request_wms_can_delete_compatibility` private. `material_issue_can_process` đã được retire và `cancel_material_issue_order` không còn tự kiểm legacy; compatibility được cô lập trong các helper được đặt tên rõ. Con số chưa giảm thêm vì shell persona hiện hành vẫn phải được bảo toàn cho tới khi mapping/reconciliation toàn cohort hoàn tất. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E15 đã hoàn tất các consumer WMS action-specific; E16 loại phương án suy quyền trả NCC từ `wms.transaction.create`; E17 cô lập xóa yêu cầu WMS nhưng chưa retire vì chưa có action canonical tương đương. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility quyết toán/hoàn tác, trả NCC, xóa yêu cầu và dữ liệu dùng chung.
