...
Sau E16 vẫn còn 11 function gọi trực tiếp literal `is_module_admin('WMS')`, nhưng `material_issue_can_process` đã được retire và `cancel_material_issue_order` không còn tự kiểm legacy; compatibility được cô lập trong `material_issue_has_process_compatibility_access` và `material_issue_can_cancel`. Con số chưa giảm vì các shell persona hiện hành vẫn phải được bảo toàn cho tới khi mapping/reconciliation toàn cohort hoàn tất. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E15 đã hoàn tất guard catalog, helper canonical-only, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi, command đổi trạng thái/điều chỉnh số lượng, đồng bộ fulfillment, wrapper nhận hàng PO theo stage, tạo/gửi/hủy phiếu xuất cấp và tách nhận/hoàn/quyết toán theo action. E16 loại phương án suy quyền trả NCC từ `wms.transaction.create` bằng reconciliation Cloud. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility quyết toán/hoàn tác, trả NCC, xóa yêu cầu và dữ liệu dùng chung.
