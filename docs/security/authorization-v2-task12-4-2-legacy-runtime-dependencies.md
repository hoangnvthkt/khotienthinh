...
| Phiếu xuất vật tư | `material_issue_can_process`, `material_issue_can_view`, `material_issue_can_manage_project`, `submit_material_issue_order`, `cancel_material_issue_order` | `material_issue_can_view` đã nối `wms.transaction.view`; tạo/gửi phiếu đã nối canonical-only `wms.transaction.create` đúng kho nguồn và giữ nguyên quyền người lập/Room/legacy persona. Nhận, hoàn, quyết toán và hủy còn phải tách riêng vì dùng chung helper người lập/người phụ trách/người nhận nhưng không cùng một capability nghiệp vụ. |
...
Sau E13 còn 11 function gọi trực tiếp literal `is_module_admin('WMS')`, chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E13 đã hoàn tất guard catalog, helper canonical-only, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi, command đổi trạng thái/điều chỉnh số lượng, đồng bộ fulfillment, wrapper nhận hàng PO theo stage và tạo/gửi phiếu xuất cấp. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại các consumer nhận/hoàn/quyết toán/hủy phiếu xuất, trả NCC, xóa yêu cầu và dữ liệu dùng chung.
