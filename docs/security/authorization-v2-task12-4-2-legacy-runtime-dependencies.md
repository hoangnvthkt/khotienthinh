...
Sau E12 còn 12 function gọi trực tiếp literal `is_module_admin('WMS')`, chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E12 đã hoàn tất guard catalog, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi, command đổi trạng thái/điều chỉnh số lượng, đồng bộ fulfillment và wrapper nhận hàng PO theo stage. Số direct dependency chưa giảm ở E12 vì helper compatibility vẫn được giữ có chủ đích; chưa loại các consumer xử lý phiếu xuất, trả NCC, xóa yêu cầu và dữ liệu dùng chung.
