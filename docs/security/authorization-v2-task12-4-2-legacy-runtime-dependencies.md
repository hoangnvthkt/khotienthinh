...
| PO và nhận hàng | `current_user_can_receive_purchase_batch_v2`, `create_purchase_order_supplier_return`, `process_transaction_status`, `sync_fulfillment_receipt_for_transaction`, `update_transaction_items_for_receipt` | Status, điều chỉnh số lượng và sync fulfillment đã dùng capability đúng kho/actor. Bốn wrapper duyệt SL/CL và xác nhận nhập đã tách `approve`/`complete`; implementation riêng tư không còn callable trực tiếp, còn helper nhận hàng giữ legacy compatibility tới lúc chuyển shell. E16 reconciliation xác nhận `create_purchase_order_supplier_return` vẫn phải manual review: RPC hiện rộng hơn UI ở 285/840 quyết định, còn cộng `wms.transaction.create` mở thêm 15 quyết định đều ngoài UI. Chưa có action trả NCC riêng và không được suy từ quyền tạo giao dịch WMS. |
...
Sau E18 còn 10 function gọi trực tiếp literal `is_module_admin('WMS')`: hai helper delete cũ không còn lặp fallback, thay bằng một `material_request_wms_can_delete_compatibility` private. `material_issue_can_process` đã được retire và `cancel_material_issue_order` không còn tự kiểm legacy; compatibility được cô lập trong các helper được đặt tên rõ. Con số chưa giảm thêm vì shell persona hiện hành vẫn phải được bảo toàn cho tới khi mapping/reconciliation toàn cohort hoàn tất. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E15 đã hoàn tất các consumer WMS action-specific; E16 loại phương án suy quyền trả NCC từ `wms.transaction.create`; E17 cô lập xóa yêu cầu WMS; E18 bổ sung capability riêng cho quyết toán/hoàn tác mà không retire persona compatibility. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility trả NCC, xóa yêu cầu, quyết toán/hoàn tác và dữ liệu dùng chung khỏi shell cũ.
