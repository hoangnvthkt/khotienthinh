...
| PO và nhận hàng | `current_user_can_receive_purchase_batch_v2`, `create_purchase_order_supplier_return`, `process_transaction_status`, `sync_fulfillment_receipt_for_transaction`, `update_transaction_items_for_receipt` | `process_transaction_status` đã nối `wms.transaction.approve/complete` và đóng bypass requester tự duyệt/tài khoản thường hoàn tất theo `assigned_warehouse_id`; requester vẫn tự hủy, Admin/module-admin/keeper tiếp tục qua compatibility trong `wms_has_action`. Bốn consumer còn lại cần đối chiếu riêng theo actor và trạng thái chứng từ. |
...
Sau E9 còn 14 function có tham chiếu WMS và `is_module_admin` trực tiếp, chưa tính toàn bộ policy và caller gián tiếp. E5–E9 đã hoàn tất guard catalog, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi và command đổi trạng thái phiếu kho; chưa loại các consumer xử lý phiếu xuất, bốn luồng PO/nhận hàng còn lại, xóa yêu cầu và dữ liệu dùng chung.
