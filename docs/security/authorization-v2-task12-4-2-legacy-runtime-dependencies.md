...
| PO và nhận hàng | `current_user_can_receive_purchase_batch_v2`, `create_purchase_order_supplier_return`, `process_transaction_status`, `sync_fulfillment_receipt_for_transaction`, `update_transaction_items_for_receipt` | `process_transaction_status`, `update_transaction_items_for_receipt` và `sync_fulfillment_receipt_for_transaction` đã dùng `wms.transaction.approve/complete` theo đúng kho tác nghiệp nguồn/đích. Requester vẫn chỉ tự hủy; status/sync đều suy actor từ JWT và không nhận ID giả mạo. Admin/module-admin/keeper tiếp tục qua compatibility trong `wms_has_action`. Hai consumer `current_user_can_receive_purchase_batch_v2` và `create_purchase_order_supplier_return` còn phải đối chiếu riêng. |
...
Sau E10 còn 12 function gọi trực tiếp literal `is_module_admin('WMS')`, chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E10 đã hoàn tất guard catalog, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi, command đổi trạng thái/điều chỉnh số lượng và đồng bộ nhận hàng; chưa loại các consumer xử lý phiếu xuất, hai luồng PO/nhận hàng còn lại, xóa yêu cầu và dữ liệu dùng chung.
