...
| Phiếu xuất vật tư | `material_issue_has_process_compatibility_access`, bốn helper action-specific nhận/hoàn/quyết toán/hoàn tác, `material_issue_can_view`, `material_issue_can_manage_project`, `submit_material_issue_order`, `cancel_material_issue_order` | `material_issue_can_view` đã nối `wms.transaction.view`; tạo/gửi phiếu đã nối canonical-only `wms.transaction.create` đúng kho nguồn. E14 đã tách helper xử lý: nhận hàng dùng thêm `wms.transaction.complete`, tạo hoàn trả dùng thêm `wms.transaction.create`, đều đúng kho nguồn và giữ creator/responsible/employee recipient/Admin/thủ kho. Quyết toán và hoàn tác đã có boundary riêng nhưng vẫn dùng compatibility, tiếp tục `manual_review` vì chưa có capability tương đương được duyệt. Hủy phiếu trước xuất vẫn phải tách riêng. |
...
Sau E14 vẫn còn 11 function gọi trực tiếp literal `is_module_admin('WMS')`, nhưng `material_issue_can_process` đã được retire và thay bằng `material_issue_has_process_compatibility_access`; con số không giảm vì compatibility cho quyết toán/hoàn tác chưa có mapping an toàn để thu hồi. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E14 đã hoàn tất guard catalog, helper canonical-only, canonical-only Hủy duyệt, hai read helper, tách policy attachment đọc/ghi, command đổi trạng thái/điều chỉnh số lượng, đồng bộ fulfillment, wrapper nhận hàng PO theo stage, tạo/gửi phiếu xuất cấp và tách nhận/hoàn/quyết toán theo action. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility quyết toán/hoàn tác, consumer hủy phiếu xuất, trả NCC, xóa yêu cầu và dữ liệu dùng chung.
