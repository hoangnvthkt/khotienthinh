...
| Xóa yêu cầu | `material_request_can_delete_v3`, `material_request_can_delete_v2`, compatibility `material_request_wms_can_delete_compatibility` | v3 giữ nhánh Project theo Room action và chỉ gọi v2 cho WMS. E17 đã gom predicate WMS vào helper private, đóng direct client EXECUTE của v1/v2 và xác nhận exact parity trên 16.800 quyết định; catalog WMS vẫn chưa có action delete riêng nên compatibility chưa thể retire. |
...
Sau E19 còn 10 function gọi trực tiếp literal `is_module_admin('WMS')`: hai helper delete cũ không còn lặp fallback, thay bằng một `material_request_wms_can_delete_compatibility` private. `material_issue_can_process` đã được retire và `cancel_material_issue_order` không còn tự kiểm legacy; compatibility được cô lập trong các helper được đặt tên rõ. Con số chưa giảm thêm vì shell persona hiện hành vẫn phải được bảo toàn cho tới khi mapping/reconciliation toàn cohort hoàn tất. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E15 đã hoàn tất các consumer WMS action-specific; E16 loại phương án suy quyền trả NCC từ `wms.transaction.create`; E17 cô lập xóa yêu cầu WMS; E18 bổ sung capability riêng cho quyết toán/hoàn tác; E19 bổ sung capability riêng cho trả NCC và giữ persona compatibility. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility trả NCC, xóa yêu cầu, quyết toán/hoàn tác và dữ liệu dùng chung khỏi shell cũ.
