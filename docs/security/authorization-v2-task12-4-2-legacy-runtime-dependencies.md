...
| Phiếu xuất vật tư | `material_issue_has_process_compatibility_access`, bốn helper action-specific nhận/hoàn/quyết toán/hoàn tác, `material_issue_can_cancel`, `material_issue_can_view`, `material_issue_can_manage_project`, `submit_material_issue_order` | `material_issue_can_view` đã nối `wms.transaction.view`; tạo/gửi phiếu đã nối canonical-only `wms.transaction.create` đúng kho nguồn. E14 đã tách helper xử lý: nhận hàng dùng thêm `wms.transaction.complete`, tạo hoàn trả dùng thêm `wms.transaction.create`, đều đúng kho nguồn và giữ creator/responsible/employee recipient/Admin/thủ kho. E18 thêm riêng `wms.material_issue.settle` và `wms.material_issue.reverse_settlement` đúng kho; không suy từ `complete/reverse`, hoàn tác là sensitive và grant trực tiếp phải có expiry. E15 nối hủy trước xuất với `wms.transaction.approve` đúng kho và người lập; `wms.transaction.reverse` không mở hành động này. |
| Xóa yêu cầu | `material_request_can_delete_v3`, `material_request_can_delete_v2`, action helper `material_request_wms_can_delete`, compatibility `material_request_wms_can_delete_compatibility` | v3 giữ nguyên nhánh Project theo Room action và chỉ gọi v2 cho WMS. E20 thêm `wms.request.delete` scope global/warehouse cho WMS ở `DRAFT/PENDING/REJECTED`, khớp kho nguồn hoặc kho công trường; nhánh capability không mở `APPROVED`, và quyền `wms.request.approve` chung không mở xóa. Predicate cũ vẫn là nhánh compatibility riêng để không làm mất persona hiện hành; 16.800 quyết định Cloud có 0 legacy loss/0 unexpected change. |
| PO và nhận hàng | `current_user_can_receive_purchase_batch_v2`, `purchase_order_supplier_return_can_create`, `create_purchase_order_supplier_return`, `process_transaction_status`, `sync_fulfillment_receipt_for_transaction`, `update_transaction_items_for_receipt` | Status, điều chỉnh số lượng và sync fulfillment đã dùng capability đúng kho/actor. Bốn wrapper duyệt SL/CL và xác nhận nhập đã tách `approve`/`complete`; implementation riêng tư không còn callable trực tiếp, còn helper nhận hàng giữ legacy compatibility tới lúc chuyển shell. E19 thêm `wms.purchase_order.return_supplier` đúng kho, đồng bộ UI/RPC và tiếp tục giữ Admin/WMS module-admin/thủ kho tổng/Project PO manager hiện hành. `wms.transaction.create` vẫn không mở trả NCC; 285/840 quyết định RPC rộng hơn UI là compatibility đã biết, chưa retire. |
...
Sau E20 còn 10 function gọi trực tiếp literal `is_module_admin('WMS')`: hai helper delete cũ không còn lặp fallback, thay bằng một `material_request_wms_can_delete_compatibility` private; action helper mới chỉ cộng capability canonical chính xác. `material_issue_can_process` đã được retire và `cancel_material_issue_order` không còn tự kiểm legacy; compatibility được cô lập trong các helper được đặt tên rõ. Con số chưa giảm thêm vì shell persona hiện hành vẫn phải được bảo toàn cho tới khi mapping/reconciliation toàn cohort hoàn tất. Chưa tính ba function có chuỗi tham chiếu rộng hơn, toàn bộ policy và caller gián tiếp. E5–E15 đã hoàn tất các consumer WMS action-specific; E16 loại phương án suy quyền trả NCC từ `wms.transaction.create`; E17 cô lập xóa yêu cầu WMS; E18 bổ sung capability riêng cho quyết toán/hoàn tác; E19 bổ sung capability riêng cho trả NCC; E20 bổ sung capability riêng cho xóa yêu cầu WMS. `submit_material_issue_order` không còn tự gọi legacy admin; `material_issue_can_manage_project` vẫn giữ compatibility có chủ đích. Chưa loại compatibility trả NCC, xóa yêu cầu, quyết toán/hoàn tác và dữ liệu dùng chung khỏi shell cũ.
