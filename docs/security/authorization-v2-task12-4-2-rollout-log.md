...
### E36 resumed preflight — cleanup đã đạt, session gate vẫn đóng

- Worktree Authorization được fast-forward `--ff-only` từ `dde5151` lên `bc48b4d`, khớp `origin/main`; Cloud linked ref là `ftciqmqhmfvjtwoycswe`. Migration dry-run trả `upToDate=true`, không có migration/seed/role chờ apply; baseline giữ `91 active / 402 archived`.
- Vercel Production deployment mới nhất ở trạng thái `READY`, target `production`, chạy đúng SHA `bc48b4d197047c847244d51c25ce1a7b916c4b24`. Release này chứa ba hotfix navigation và fix capability Tài sản cần cho E36.
- Checker Hương-only lúc `2026-09-18 05:00:30 UTC` xác nhận owner cleanup đã hoàn tất: `0 workflow.*` direct grant, `61` grant ngoài Workflow với fingerprint không đổi `ebb0b31ff1a5397d49f6c82a9aa6be3fa25c35d252b40b4d6e1ab30aedf9abdc`, và compatibility shell `system.wf.view` vẫn được giữ. Chín Workflow grant cũ đều đã revoke lúc `03:15:05 UTC`; audit có `direct_permission_grants_changed` và `user_authorization_v2_updated`. Không gửi lại command revoke.
- Templates vẫn `WORKFLOW_USER=6 items`, `WORKFLOW_ADMIN=12 items`; active Workflow assignment bằng `0`, assignment cũ của Hương ở `REVOKED`, transition ledger `0 batch / 0 item`, và không có Workflow command/instance/log mới trong cửa sổ evidence.
- Production có một Permission Admin với session active, nhưng Hương có `0` active session và `0` refresh trong hai giờ gần nhất tại thời điểm checker (dù `last_sign_in_at` là `04:19:51 UTC`). Vì vậy maintenance gate vẫn đóng; chưa preview/assign `WORKFLOW_USER` và chưa có mutation Cloud trong checkpoint này. Chờ Hương đăng nhập Production lại và xác nhận bắt đầu cửa sổ nghiệm thu.

### E36 PASS — Production persona Hương, bounded assignment và final revoke

- Production release được xác minh `READY` trên SHA `bc48b4d197047c847244d51c25ce1a7b916c4b24`. Sau khi Hương đăng nhập lúc `05:02:59 UTC`, checker ghi nhận 2 session active và 2 session refresh trong hai giờ gần nhất.
- Preview/assignment qua V2 dưới Permission Admin đạt: `WORKFLOW_USER` version 1, `6` quyền, `0` nhạy cảm, `0` hard deny, `0` warning, scope assignment `global/*`, expiry 24 giờ. Assignment được tạo lúc `05:06:40 UTC` và chỉ có một assignment active trong lúc nghiệm thu.
- Production allow/deny matrix đạt bằng phiên Hương trên đúng release: `/wf` mở thành công và hiển thị 13 phiếu; `/wf/templates` mở thành công và hiển thị catalog mẫu; truy cập trực tiếp `/wf/templates/new` bị route guard đưa về Home. Không tạo/lưu nháp hoặc thay đổi dữ liệu nghiệp vụ thật.
- Reconciliation sau assignment có đủ `6/6` ROLE sources, `0` direct `workflow.*`, compatibility shell `system.wf.view` được giữ, 61 direct grant ngoài Workflow giữ fingerprint `ebb0b31ff1a5397d49f6c82a9aa6be3fa25c35d252b40b4d6e1ab30aedf9abdc`, không có Workflow command/instance/log mới.
- Assignment Hương được revoke ngay sau evidence qua `revoke_business_role_assignment`, audit `business_role_revoked` lúc `06:42:50 UTC`. Final postflight: assignment ở `REVOKED`, active assignment `0`, templates `WORKFLOW_USER=6`/`WORKFLOW_ADMIN=12`, transition ledger `0 batch / 0 item`.
- E36 được ghi `PASS`. Không mở observation T0, không revoke compatibility shell, không thay đổi 13 cohort `owner_pending`, và Task 13 vẫn bị chặn theo runbook.

## E37 — Workflow participant continuity, canonical deep links và server worker

- Đã triển khai ba forward migration cho generic Workflow: participant ledger/RLS/backfill, participant-based notification outbox/delivery, và forward-fix resolver để assignment event lấy assignee theo node hiện tại sau transition. Request/Project/Material-owned workflow vẫn bị suppress khỏi generic outbox; không thay đổi role/capability hay Task 13.
- Cloud preflight đúng project main `ftciqmqhmfvjtwoycswe`; notification gate ban đầu `enabled=false`. Dry-run trước apply chỉ liệt kê hai migration participant/notification. Sau khi sửa lỗi resolver bằng forward migration, Cloud dry-run trả up-to-date và baseline Git là `94 active / 402 archived`.
- Rollback smoke đã đạt với RLS/ACL participant, backfill continuity, selector contract, dedup recipient và event-key idempotency. Bằng chứng Cloud đã lược PII cho `WF-2026-172`: `9` participant role-row đang active và prior actor continuity là `true`. Participant table có RLS, `anon/authenticated/service_role` đều không có direct SELECT.
- Canonical route `/wf/:instanceId`, redirect legacy `/wf/instances/:id` và query legacy, direct loader vượt cache 300, link mã `WF-...`, copy-link action và notification resolver đều đã nối vào frontend. Generic React fan-out đã bị loại; chỉ còn specialized Material/Request path. Notification worker Edge Function đã deploy với secret-boundary handler, health `ok=true`, và gate vẫn tắt.
- Verification checkpoint: Vitest `422/422` files, `1993/1993` tests; TypeScript, production build, migration baseline, Cloud rollback smoke và `git diff --check` đều đạt. Outbox backlog cũ của `WF-2026-172` có `3` event; sau khi ghi `rollout_started_at` và claim một lần trong lúc gate tắt, cả `3` đã được suppress với `pre_rollout_backlog`.
- Chưa bật gate production vì frontend canonical route chưa được deploy lên Production trong checkpoint này. Bước tiếp theo sau khi merge/deploy đúng release là canary với persona đã chỉ định, xác nhận recipient/actionable/deeplink, rồi mới bật `workflow_notification_settings.enabled`; nếu lỗi, tắt gate mà vẫn giữ participant visibility và canonical route.
