...
- The G6 WMS inventory workspace already separates on-hand, reserved, available, in-transit transfer, and receipt-custody quantities, and marks availability unknown when the stock cache, ledger, or count evidence disagrees. Use that authoritative availability rule for a future input warning and pending-goods detail; do not substitute BOQ remainder or silently coerce an unresolved stock balance to zero.
