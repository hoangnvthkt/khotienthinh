...
Chủ dự án chọn hai cặp thử đầu tiên: `Đổ bê tông lót → Bê tông M300 (100 m³)` và `Đào móng → Cát trát (100 tấn)`. Chưa ghi chúng thành định mức: cần làm rõ 100 là tổng vật tư hay suất tiêu hao trên một đơn vị công tác và nguồn phê duyệt. BOQ thủ công hiện ghi `Cát trát` bằng `m³`, khác đơn vị `tấn` mới nêu; không tự đổi đơn vị hoặc suy diễn hệ số. Check migration baseline ngày 25/09 còn fail với 14 file post-baseline chưa allowlist.
